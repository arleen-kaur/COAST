"""Build MovieLens meta.csv from movies.dat + TMDB plot summaries."""
...
def build_meta_without_tmdb(cfg, out_path=None):
    """Fallback when TMDB_API_KEY is not set."""
    needed = needed_movie_ids(cfg)
    movies = load_movies_dat(cfg)
...
def main():
    ...
    if not cfg.movies_dat().is_file():
        raise FileNotFoundError(f"run download_movielens.py first")
