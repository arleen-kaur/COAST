...
# SASRec-style transformer blocks, but item vectors = ID embedding + projected MiniLM text
...
class COAST(torch.nn.Module):
    ...
    def item_vec(
        ...
        # zero out ID part for items never seen in training (cold items in history)
    ...
    def predict(
        ...
        # for cold-start eval, score all candidates with content only (no ID lookup)
